...
ConstantBuffer<interlop::ProbeDistanceBlendingCSResources> cs_resources : register(b0);
          ...
          uint  group_index        : SV_GroupIndex)
{
  ...
  RWTexture2DArray<float2>              distance = ResourceDescriptorHeap[cs_resources.distance];
  ...
  if (!is_border_texel)
  {
    float2 probe_octant_uv = (group_thread_id.xy - 1.0f) / (float)kProbeNumDistanceInteriorTexels;
    ...
    for (int iray = 0; iray < vol_desc.probe_num_rays; iray++)
    {
      ...
      sum += float3(sample_distance * weight, (sample_distance * sample_distance) * weight, weight);
    }
    ...
    //
    // (I have no idea what the purpose of the epsilon is here, something about division by 0 I think)
    float epsilon = float(vol_desc.probe_num_rays) * 1e-9f;
    sum.rg       *= 1.0f / (2.0f * max(sum.b, epsilon));

    // Get the current distance of the probe
    float2 probe_distance = distance[dispatch_thread_id].rg;
    ...
    distance[dispatch_thread_id] = lerp(sum.rg, probe_distance, hysteresis);
    return;
  }
  ...
  bool is_corner_texel = (group_thread_id.x == 0 || group_thread_id.x == (kProbeNumDistanceTexels - 1)) && 
                         (group_thread_id.y == 0 || group_thread_id.y == (kProbeNumDistanceTexels - 1));
  bool is_row_texel    = (group_thread_id.x > 0 && group_thread_id.x < (kProbeNumDistanceTexels - 1));
  uint3 src_coords     = uint3(group_id.x * kProbeNumDistanceTexels,
                               group_id.y * kProbeNumDistanceTexels,
  ...
  {
    src_coords.x += (kProbeNumDistanceTexels - 1) - group_thread_id.x;
