...
for /f %%f in ('dir /b %in_vertex_dir%') do (%dxc% -T vs_6_6 -E main %in_vertex_dir%\%%f -Fo %out_vertex_dir%\%%f.bin -Od || goto compilation_error)
for /f %%f in ('dir /b %in_pixel_dir%') do (%dxc% -T ps_6_6 -E main %in_pixel_dir%\%%f -Fo %out_pixel_dir%\%%f.bin -Od || goto compilation_error)
